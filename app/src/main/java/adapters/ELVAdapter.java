...
public class ELVAdapter extends BaseExpandableListAdapter {
    ...
    */
    public ELVAdapter(Context context, ArrayList<String> listDataGroup, HashMap<String, List<String>> listDataChild) {
    ...
    public int getNumberOfCheckedItemsInGroup(int mGroupPosition) {
        ...
        int count = 0;
        if (getChecked != null) {
            for (boolean aGetChecked : getChecked) {
                if (aGetChecked) count++;
            }
        }
        return count;
    }

    public ArrayList<String> getCheckedItems(int mGroupPosition) {
        ArrayList<String> result = new ArrayList<>();
        boolean getChecked[] = mChildCheckStates.get(mGroupPosition);
        if (getChecked != null) {
            for (int i = 0; i < getChecked.length; i++) {
                if (getChecked[i]) {
                    result.add(mListDataChild.get(mListDataGroup.get(mGroupPosition)).get(i));
                }
            }
        }
        return result;
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            childViewHolder.mChildText = (TextView) convertView
                    .findViewById(R.id.elv_ListItem);

            childViewHolder.mCheckBox = (CheckBox) convertView
                    .findViewById(R.id.elv_child_checkbox);
